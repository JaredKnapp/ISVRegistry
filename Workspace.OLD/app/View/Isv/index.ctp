				...
				<tr>
					<td width="40%">
						<table>
							...
							<tbody>
								<tr><td><?php echo $this->Form->text("partners-name", array('placeholder' => "Partner names...", 'default' => '')); ?></td></tr>
								<tr><td><?php echo $this->Form->text("products-name", array('placeholder' => "Product names...", 'default' => '')); ?></td></tr>
								<tr><td><?php echo $this->Form->text("industries-name", array('placeholder' => "Industries...", 'default' => '')); ?></td></tr>
								<tr><td><?php echo $this->Form->text("workloads-name", array('placeholder' => "Workloads...", 'default' => '')); ?></td></tr>
								<tr>
									<td>
	<?php 
...
foreach($platforms as $index=>$platform){
	...
	if($isVisible){
		echo '      <th>' . $this->Paginator->sort($platformindex, $platform['platforms']['name'] . ' ' . $platform['platformversions']['version']) . '</th>'."\r\n";
	}
}
...
<?php foreach ($validations as $validation): ?>
	...
	if(empty($validation['partners']['partner_url'])){
		echo h($validation['partners']['partner']); 
	} else {
		echo $this->Html->link(h($validation['partners']['partner']), $validation['partners']['partner_url'], array('target' => '_blank')); 
	}
	?></td>
	<td><?php 
	if(empty($validation['products']['product_url'])){
		echo h($validation['products']['product']);
	} else {
		echo $this->Html->link(h($validation['products']['product']), $validation['products']['product_url'], array('target' => '_blank')); 
	}
	?></td>
	<td><?php echo h($validation['versions']['version']); ?></td>
	<td><?php echo h($validation['industries']['industry']); ?></td>
	<td><?php echo h($validation['workloads']['workload']); ?></td>
	<?php
	foreach($platforms as $index=>$platform){
		...
		if($isVisible){
			...
			if(is_null($status)){
				...
				echo '<div class="statusoval" title="'.$statuses[$status]['label'].'" style="background-color:'.$statuses[$status]['color']['back'].'; color:'.$statuses[$status]['color']['fore'].'; border-color:'.$statuses[$status]['color']['border'].';">'.$level.'</div>';
			}
			echo '</td>';
		}
	}
	...
	if (AuthComponent::user('id') && (AuthComponent::user('role')==='editor' || AuthComponent::user('role')==='admin')) {
		...
		echo $this->Html->link( 'Edit', array('controller' => 'Partners', 'action' => 'productEdit', $validation['products']['product_id']) );
		echo '</td>';
	}
...
$('#FilterIndexForm :checkbox').click(function() {
  var form = this;
  clearTimeout(timerid);
  timerid = setTimeout(function() { $('#FilterIndexForm').submit(); }, delay);
});
